...
`datatype = "abundance Data"` is used for a random sampling scheme. If the species has aggregation effect, such as trees or plants, please use `datatype = "incidence_raw"`. First, we use data `Data_spider` to compute taxonomic diversity.
...
* Assemblage = the datasets you selected in the 'Data setting' on the left hand side of the screen.
...
* TD_obs = the empirical diversity profile of order q.
* TD_asy = the estimated asymptotic diversity profile of order q.
...
`$details` contains four parts: `Sample completeness`, `iNEXT`, `Observed and asymptotic diversity`, `Evenness`. They are the computing values which are used to plot each figure in `$figure`. 
